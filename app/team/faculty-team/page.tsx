...
export default function FacultyTeamPage() {
  const categoryToFilter: TeamCategory = "Faculty";
  const pageTitle = "Faculty Team";
  const pageDescription =
    "Meet the accomplished IIT Madras faculty members driving the core research and academic excellence within the FedEx SMART Centre's key verticals.";

  // Filter team data for this specific category
  const facultyTeam = teamData.filter(
        ...
        {facultyTeam.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {facultyTeam.map((member) => {
              ...
              if (!personDetails) {
                console.warn(
                  `Data integrity issue: Person details not found for ID: ${member.personId} on Faculty page.`
                ); // Optional warning
                return null;
              }
          ...
          <p className="text-center text-muted-foreground py-10">
            Faculty team information will be updated soon.
          </p>
        )}
      </section>
    </main>
